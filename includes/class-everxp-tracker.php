...
class EverXP_Tracker {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_track_event', [$this, 'log_event']);
        add_action('wp_ajax_nopriv_track_event', [$this, 'log_event']); // Allow guest tracking
    ...
    /**
     * Handle AJAX request for event tracking (custom events)
     */
    public function log_event() {
        ...
        $user_identifier = self::everxp_get_user_identifier();
        $utm_parameters = $data['eventData']['utm_parameters'] ?? [];

        if (empty($data['eventType']) || empty($data['eventData'])) {
            wp_send_json_error(['message' => 'Invalid data'], 400);
        }

        if (!$this->is_everxp_attributed($data['eventData']['utm_parameters'] ?? [])) {
            wp_send_json_error(['message' => 'Not attributed to EverXP'], 403);
        }
        ...
        $wpdb->insert("{$wpdb->prefix}api_user_logs", [
            ...
            'utm_parameters'  => json_encode($data['eventData']['utm_parameters'] ?? []),
            'timestamp'       => time(),
